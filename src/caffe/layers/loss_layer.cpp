...
template <typename Dtype>
void PSNRLossLayer<Dtype>::SetUp(
  const vector<Blob<Dtype>*>& bottom, vector<Blob<Dtype>*>* top) {
  CHECK_EQ(bottom.size(), 2) << "Loss Layer takes two blobs as input.";
  CHECK_EQ(top->size(), 2) << "Loss Layer takes two blobs as output."; // psrn & restored image
  CHECK_EQ(bottom[0]->num(), bottom[1]->num())
	<< "The data and label should have the same number.";
  CHECK_EQ(bottom[0]->channels(), bottom[1]->channels());
  CHECK_EQ(bottom[0]->height(), bottom[1]->height());
...
template <typename Dtype>
Dtype PSNRLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  vector<Blob<Dtype>*>* top) {
  int count = bottom[0]->count();
  int num = bottom[0]->num();
  caffe_sub(count, bottom[0]->cpu_data(), bottom[1]->cpu_data(),
  ...
  Dtype scale = Dtype(1); // maximum value in image
  if (this->layer_param_.data_param().has_scale() &&
	this->layer_param_.data_param().scale() > kLOG_THRESHOLD) {
	scale = Dtype(this->layer_param_.data_param().scale());
  }
  Dtype mse = scale * scale * loss / (bottom[0]->channels() * bottom[0]->height() * bottom[0]->width());
  Dtype psnr = 10 * (- log10(mse));
  (*top)[0]->mutable_cpu_data()[0] = mse;
  (*top)[0]->mutable_cpu_data()[1] = psnr;
  caffe_copy(bottom[0]->count(), bottom[0]->cpu_data(), (*top)[1]->mutable_cpu_data());
  return loss;
}

template <typename Dtype>
void PSNRLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  const bool propagate_down, vector<Blob<Dtype>*>* bottom) {
  int count = (*bottom)[0]->count();
  int num = (*bottom)[0]->num();
  // Compute the gradient
  caffe_cpu_axpby(count, Dtype(2) / num, difference_.cpu_data(), Dtype(0),
	(*bottom)[0]->mutable_cpu_diff());
}
...
INSTANTIATE_CLASS(PSNRLossLayer);
