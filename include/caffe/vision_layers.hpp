...
protected:
  ...
  shared_ptr<SyncedMemory> diff_multiplier_;
  int num_;
  int channels_;
  int height_;
  int width_;
...
template <typename Dtype>
class NoisyDataLayer : public Layer<Dtype> {
  // The function used to perform prefetching.
  friend void* NoisyDataLayerPrefetch<Dtype>(void* layer_pointer);

public:
  explicit NoisyDataLayer(const LayerParameter& param)
	: Layer<Dtype>(param) {}
  virtual ~NoisyDataLayer();
  virtual void SetUp(const vector<Blob<Dtype>*>& bottom,
	vector<Blob<Dtype>*>* top);
...
template <typename Dtype>
class PSNRLossLayer : public Layer<Dtype> {
public:
  explicit PSNRLossLayer(const LayerParameter& param)
	: Layer<Dtype>(param), difference_() {}
  virtual void SetUp(const vector<Blob<Dtype>*>& bottom,
	vector<Blob<Dtype>*>* top);
