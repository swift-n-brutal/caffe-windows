...
template <typename Dtype>
Dtype caffe_rng_gaussian(const Dtype a, const Dtype sigma) {
  CHECK_GT(sigma, 0);
  boost::normal_distribution<Dtype> random_distribution(a, sigma);
  boost::variate_generator<caffe::rng_t*, boost::normal_distribution<Dtype> >
	variate_generator(caffe_rng(), random_distribution);
...
template
float caffe_rng_gaussian<float>(const float a, const float sigma);

template
double caffe_rng_gaussian<double>(const double a, const double sigma);
