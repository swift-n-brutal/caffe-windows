...
class XavierFiller : public Filler<Dtype> {
 public:
  ...
  virtual void Fill(Blob<Dtype>* blob) {
    ...
    // Dtype scale = sqrt(Dtype(3) / sqrt(fan_in));
	// int fan_in_out = blob->height() + blob->width();
	// Dtype scale = sqrt(Dtype(6) / fan_in_out);
    // caffe_rng_uniform<Dtype>(blob->count(), -scale, scale,
    //    blob->mutable_cpu_data());
	caffe_rng_gaussian<Dtype>(blob->count(), Dtype(0.0),
		Dtype(1.0 / sqrt(fan_in)), blob->mutable_cpu_data());
    CHECK_EQ(this->filler_param_.sparse(), -1)
         << "Sparsity not supported by this Filler.";
  }
};

// Xavier-like filler for convolutional layer
template <typename Dtype>
class XconvFiller : public Filler<Dtype> {
public:
  explicit XconvFiller(const FillerParameter& param)
	: Filler<Dtype>(param) {}
  virtual void Fill(Blob<Dtype>* blob) {
	CHECK(blob->count());
	int fan_in = blob->count() / blob->num();
	caffe_rng_gaussian<Dtype>(blob->count(), Dtype(0.0),
	  Dtype(sqrt(2.0 / fan_in)), blob->mutable_cpu_data());
	CHECK_EQ(this->filler_param_.sparse(), -1)
	  << "Sparsity not supported by this Filler.";
  }
};
...
Filler<Dtype>* GetFiller(const FillerParameter& param) {
  ...
  if (type == "constant") {
	...
	return new XconvFiller<Dtype>(param);
  } else {
