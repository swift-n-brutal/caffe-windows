...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  while (iter_++ < param_.max_iter()) {
    ...
    if (param_.display() && iter_ % param_.display() == 0) {
	  ...
	  if (param_.display_verbose()) {
		ShowParamsAndGradients();
		ShowBlobs();
	  }
    }
  ...
  for (int i = 0; i < param_.test_iter(); ++i) {
	...
	if (i == 0) {
	  for (int j = 0; j < result.size(); ++j) {
		if (result[j]->count() > kImageThres) {
		  // the result is possibly an image
		  continue;
		}
...
void SGDSolver<Dtype>::ComputeUpdateValue() {
  ...
  case Caffe::GPU:
	for (int param_id = 0; param_id < net_params.size(); ++param_id) {
	  ...
	  Dtype local_rate = rate * net_params_lr[param_id];
	  Dtype local_decay = weight_decay * net_params_weight_decay[param_id];
	  if (net_params_lr[param_id] < FLT_MIN) {
		CUDA_CHECK(cudaMemset(history_[param_id]->mutable_gpu_data(), 0,
		  sizeof(Dtype)* history_[param_id]->count()));
	  } else {
		caffe_gpu_axpby(net_params[param_id]->count(), local_rate,
		  net_params[param_id]->gpu_diff(), momentum,
		  history_[param_id]->mutable_gpu_data());
		if (net_params_weight_decay[param_id] > FLT_MIN) {
		  // add weight decay
...
template <typename Dtype>
void SGDSolver<Dtype>::ShowParamsAndGradients() {
  LOG(INFO) << "----- Params and Gradients -----";
  const vector<string>& layer_names = this->net_->layer_names();
  for (vector<string>::const_iterator layer_iter = layer_names.begin();
	layer_iter != layer_names.end(); layer_iter++) {
	const shared_ptr<Layer<Dtype>> layer =
	  this->net_->layer_by_name(*layer_iter);
	vector<shared_ptr<Blob<Dtype>>>& params = layer->blobs();
	if (params.size()) {
	  LOG(INFO) << "Layer " << *layer_iter << " has " <<
		  params.size() << " params:";
	  if (Caffe::mode() == Caffe::GPU) {
		for (int i = 0; i < params.size(); ++i) {
		  const int count = params[i]->count();
		  Dtype asum_data, asum_diff;
		  caffe_gpu_asum(count, params[i]->gpu_data(), &asum_data);
		  caffe_gpu_asum(count, params[i]->gpu_diff(), &asum_diff);
		  LOG(INFO) << "  " << i << ": " << asum_data / count <<
			" " << asum_diff / count <<
			" (" << asum_diff / asum_data << ")";
		}
	  } else {
		for (int i = 0; i < params.size(); ++i) {
		  const int count = params[i]->count();
		  Dtype asum_data, asum_diff;
		  asum_data = caffe_cpu_asum(count, params[i]->cpu_data());
		  asum_diff = caffe_cpu_asum(count, params[i]->cpu_diff());
		  LOG(INFO) << "  " << i << ": " << asum_data / count <<
			" " << asum_diff / count <<
			" (" << asum_diff / asum_data << ")";
		}
	  ...
	  LOG(INFO) << "Layer " << *layer_iter << " has no params.";
	}
  }
}

template <typename Dtype>
void SGDSolver<Dtype>::ShowBlobs() {
  LOG(INFO) << "----- Blobs -----";
  const vector<string>& blob_names = this->net_->blob_names();
  for (vector<string>::const_iterator blob_iter = blob_names.begin();
	  blob_iter != blob_names.end(); blob_iter++) {
	const shared_ptr<Blob<Dtype>> blob =
	    this->net_->blob_by_name(*blob_iter);
	if (Caffe::mode() == Caffe::GPU) {
	  const int count = blob->count();
	  Dtype asum_data;
	  caffe_gpu_asum(count, blob->gpu_data(), &asum_data);
	  LOG(INFO) << "Blob " << *blob_iter << ": " << asum_data / count;
	} else {
	  const int count = blob->count();
	  Dtype asum_data;
	  asum_data = caffe_cpu_asum(count, blob->cpu_data());
	  LOG(INFO) << "Blob " << *blob_iter << ": " << asum_data / count;
	}
  }
}
